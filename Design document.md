...
- **Settings:** Adds a dedicated appearance selector (system, light, or dark) alongside the existing “User Data” panel, backup directory/automation controls, and session defaults (now clamped to 1–100 questions) so learners can tailor both visuals and behavior. The Backups card now groups directory info, action buttons, and auto-backup preferences into clearer sections with aligned checkbox and interval controls.
- **Session Configurator:** Modal for assembling session parameters now shows the live count of questions matching the selected filters, uses a multi-select dropdown for category filtering, enforces a 1–100 question range in the picker, and prevents launching when the pool is empty.
